...
df_r_imp <- read.csv(here(dataFolder, "02_2_data_random_imputed.csv"))  %>% dplyr::select(-X)


## ------------------------------------------------------------------------------------------------------------------------------------------------
# get current clock info
df_Y <- read.csv(here("data", "DataPreprocessing", "df_Y.csv"), row.names = 1, header = T)
...
df_imp <- merge(df_Y %>% dplyr::select(ICUSTAY_ID, half_day, Y_t, Y_t_plus_1), df_imp, 
            by = c("ICUSTAY_ID", "half_day", "Y_t_plus_1"), all.y = T)
df_r_imp <- merge(df_Y %>% dplyr::select(ICUSTAY_ID, half_day, Y_t, Y_t_plus_1), df_r_imp, 
                by = c("ICUSTAY_ID", "half_day", "Y_t_plus_1"), all.y = T)

# with all blocks, including the incomplete ones
df_all <- df
df_imp_all <- df_imp
df_r_imp_all <- df_r_imp
...
df_imp$Y_t_plus_1 = ifelse(df_imp$Y_t_plus_1 == "2","0", df_imp$Y_t_plus_1)
df_r_imp$Y_t_plus_1 = ifelse(df_r_imp$Y_t_plus_1 == "2","0", df_r_imp$Y_t_plus_1)
...
write.csv(df_r_imp, file=here(dataFolder,"02_3_data_random_imputed_complt_blocks.csv"))
...
write.csv(df_r_imp_all, file=here(dataFolder,"02_3_data_random_imputed_all_blocks.csv"))
