...
table <- df %>%
  select(ICUSTAY_ID, AGE, GENDER_F, MARITAL_PARTN, LANGUAGE_ENGL, RELIGION_Chrt, ETHNICITY_W, ETHNICITY_B, ETHNICITY_A, ETHNICITY_H, ETHNICITY_O, INSURANCE_Y, Y_t_plus_1) %>%
  unique() %>% 
  merge(ICUSTAYS, by = "ICUSTAY_ID") %>%
  select(SUBJECT_ID, AGE, GENDER_F, MARITAL_PARTN, LANGUAGE_ENGL, RELIGION_Chrt, ETHNICITY_W, ETHNICITY_B, ETHNICITY_A, ETHNICITY_H, ETHNICITY_O, INSURANCE_Y, Y_t_plus_1) %>%
  unique() %>%
  tbl_summary(
...
p <- ggplot(data=results_df_tmp, aes(x=var, y=auc, ymin=ci_l_auc, ymax=ci_u_auc)) +
   ...
   labs(title = "AUC Performances in Random Forest Models
         Across Different Training Data")

p

ggsave(here("VIA_rf_AUC.png"), p, width = 7, height = 4, dpi = 1200)
