...
for(col_i in var_name_list$vitals){
  df[[col_i]][is.na(df[[col_i]])] = vitals_labs_summary$vital_summary$median[vitals_labs_summary$vital_summary$ABB == col_i]
}

for(col_i in var_name_list$labs){
  df[[col_i]][is.na(df[[col_i]])] = vitals_labs_summary$lab_summary$median[vitals_labs_summary$lab_summary$ABB == col_i]
}
...
rdataFolder <- here("data", "data_v2", "rdata")
save.image(file=here(rdataFolder, "02_DataImputation.RData"))
